...
function Is-ExternalPostgres {
    param([string]$Value)
    return $Value.StartsWith("postgres", [System.StringComparison]::OrdinalIgnoreCase)
}
...
if (-not $DatabaseUrl) {
    throw "DATABASE_URL is missing. Set it before running Render/Supabase migrations."
}

Write-Host "DATABASE_URL_MASKED=$(Mask-DatabaseUrl -Value $DatabaseUrl)"

if (Is-ExternalPostgres -Value $DatabaseUrl) {
    $Answer = Read-Host "This will run alembic upgrade head on the configured external/Postgres database. Type YES to continue"
    if ($Answer -ne "YES") {
